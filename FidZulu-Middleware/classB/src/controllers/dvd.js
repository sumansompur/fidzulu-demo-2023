...
const logger = createLoggerWithPrefix("DVD Service");
...
router.get(Constants.APPLICATION_ROUTES.DVD_ROUTES.ALL_DVDS, async(req, resp) => {
    ...
    try{
        ...
        logger.error("Could not connect to backend for getting dvd details. ERROR:\n"+e);
...
router.get(Constants.APPLICATION_ROUTES.DVD_ROUTES.DVDS_TEAM, async(req, resp) =>{
    ...
    try{
        ...
        backendResp = await axios.get(`${Constants.ENV.HOST_DVD}/teams`);
        if(backendResp.data.success){
